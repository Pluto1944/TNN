...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if flags.FLAGS.tf_quant:
      if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        quantize.experimental_create_training_graph(tf.get_default_graph(), symmetric=True, use_qdq=True)
      else:
        quantize.experimental_create_eval_graph(tf.get_default_graph(), symmetric=True, use_qdq=True)
...
def define_resnet_flags(resnet_size_choices=None, dynamic_loss_scale=False,
  ...
  flags.DEFINE_bool(
      name='tf_quant', default=False,
      help=flags_core.help_wrap(
          'Uses tensorflow.contrib.quantize for quant-aware-training. Only support Tensorflow 1.15'))
